...
var inckCol=1;var inckRow=1;for (i = 0; i < xCell.length; i++) {if ($("#ic" + inckRow.toString() + "_" + inckCol.toString()).length) {$("#ic" + inckRow.toString() + "_" + inckCol.toString()).val(DameroWords(xCell[i]).substr(0, 1));}
inckCol++;if (inckCol > colCount) {inckCol = 1;inckRow++;}}for (i = 0; i < upos[inQue].length; i++) {if ($("#dc1_" + (i + 1).toString()).length){$("#dc1_" + (i + 1).toString()).val(DameroWords(yCell[inQue]).substr(i, 1));}}
}
function paintBack(){}
